...
%arrow3 - plot a 3D arrow as patch object (cylinder+cone)
%
% syntax:   h = arrow3(p1,p2)
%           h = arrow3(p1,p2,'propertyName',propertyValue,...)
%
...
%           'facealpha'.)
%                       
% example1: h = arrow3([0 0 0],[1 1 1])
%           (Draws an arrow from [0 0 0] to [1 1 1] with default properties.)
%
% example2: h = arrow3([0 0 0],[1 1 1],'color','red','stemWidth',0.02,'facealpha',0.5)
    ...
    switch varargin{argno}
        ...
        case 'stemWidth'
            if isreal(varargin{argno+1})
                ...
                warning('arrow3:stemWidth','stemWidth must be a real number');
            end
        case 'tipWidth'
            if isreal(varargin{argno+1})
                tipWidth = varargin{argno+1};
            else
                warning('arrow3:tipWidth','tipWidth must be a real number');
            end
        otherwise
